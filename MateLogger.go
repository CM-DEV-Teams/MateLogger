...
func (l *LogFile) Rotate() error {
	...
	if _, err := os.Stat(filepath.Join(l.path, l.name+l.logDate+".log")); err == nil {
		...
		l.file.Close()
		if err = os.Rename(filepath.Join(l.path, l.name+l.logDate+".log"), filepath.Join(l.path, name)); err != nil {
			return err
		}
	}
	...
	l.innerLogSize = 0
	file, err := os.OpenFile(filepath.Join(l.path, l.name+l.logDate+".log"), os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		return err
	}
